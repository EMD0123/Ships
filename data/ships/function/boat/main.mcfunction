...
execute as @e[type=#ships:uses_boat,predicate=!ships:target,predicate=ships:on_boat] if score Game ships.gamerule.alwayspatrol matches 1 on vehicle on passengers as @s[type=minecraft:marker,tag=ships.boatai.boat] at @s rotated ~ 0 positioned ^ ^ ^10 unless predicate ships:water run rotate @s ~90 ~
execute as @e[type=#ships:uses_boat,predicate=!ships:target,predicate=ships:on_boat] if score Game ships.gamerule.alwayspatrol matches 1 on vehicle on passengers as @s[type=minecraft:marker,tag=ships.boatai.boat] at @s rotated ~ 0 positioned ^ ^.5 ^10 unless block ~ ~ ~ #minecraft:replaceable run rotate @s ~90 ~
